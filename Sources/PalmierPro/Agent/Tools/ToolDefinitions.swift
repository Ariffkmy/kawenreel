...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            ...
            description: "Analyze video usability with on-device temporal metrics before choosing good takes or trimming bad footage. Each window is measured from a burst of consecutive native-fps frames with sub-pixel image registration, so handheld shake is measured directly rather than inferred from sparse stills. Returns per-window qualityScore, stability (stable/usable/moving/shaky) with stabilityScore, clarity, sharpness, motion (global translation speed, fraction of frame width per second), jitter and peakJitter (deviation from the window's mean motion — the shake signal), visualChange, exposure (underexposed/overexposed + meanLuma and shadow/highlight clipping), isUsable, and issues such as shaky, blurry, soft focus, static, high motion, underexposed, or overexposed. bestRanges only contains clear, non-shaky windows, so if the start is blurry but the later shot becomes clear, use the later clear range and do not place the blurry section on the timeline. When several bestRanges have similar qualityScore, prefer the highest stabilityScore. This is the reliable signal for stable vs shaky footage, motion settling, focus/blur, and candidate usable ranges; inspect_media only shows sparse still frames.\n\nUse windowSeconds around 1.5–3 for take selection. Use startSeconds/endSeconds to zoom into a suspected range. Results are source seconds unless clipId is supplied; when clipId is set, frame ranges are mapped to project frames for that clip.",
            inputSchema: objectSchema(
                properties: [
                    ...
                    "entries": [
                        ...
                        "items": [
                            ...
                            "properties": [
                                ...
                                "trackIndex": ["type": "integer", "description": "Optional. Track index (0-based). Omit on every entry to reuse an existing track with free space (one is created only when none fits)."],
    ...
    static let manageProject = AgentTool(
        name: .manageProject,
        description: "List, open, create, or close Kawenreel projects for this MCP session. Set `action` to: `list` for known projects plus session-active and visible state; `open` with a name, id from list, or .palmier path; `create` with an optional name and initial fps/aspectRatio/quality; or `close` to save and close the session project, optionally targeting another open project by name/id/path. Opening or creating changes only this session's target. Closing always completes a final save first. This tool never deletes projects or files.",
        inputSchema: objectSchema(
            properties: [
