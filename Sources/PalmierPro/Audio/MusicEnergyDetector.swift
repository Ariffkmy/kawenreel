...
struct MusicEnergyAnalysis: Sendable {
    let bpm: Double
...
enum MusicEnergyDetectorError: LocalizedError {
    case noAudioTrack
...
enum MusicEnergyDetector {
    ...
    static func analyze(url: URL) async throws -> MusicEnergyAnalysis {
        let (samples, duration) = try await loadMonoPCM(url: url)
        guard samples.count >= fftSize * 2 else { throw MusicEnergyDetectorError.insufficientData }
    ...
    private static func loadMonoPCM(url: URL) async throws -> ([Float], Double) {
        ...
        guard let track = try? await asset.loadTracks(withMediaType: .audio).first else {
            throw MusicEnergyDetectorError.noAudioTrack
        }
        let assetDuration = (try? await asset.load(.duration))?.seconds ?? 0

        let reader: AVAssetReader
        do { reader = try AVAssetReader(asset: asset) }
        catch { throw MusicEnergyDetectorError.readFailed(error.localizedDescription) }
        ...
        guard reader.canAdd(output) else { throw MusicEnergyDetectorError.readFailed("Cannot configure audio output") }
        reader.add(output)
        guard reader.startReading() else {
            throw MusicEnergyDetectorError.readFailed(reader.error?.localizedDescription ?? "reader failed to start")
        }
